...
greedy_knapsack <- function(x, W) {
  ...
  for (current_object in 1:number_of_objects) {
    ...
    if (current_weight + x[[current_object, "w"]] <= W) {
      current_weight <- current_weight + x[[current_object, "w"]]
      current_value <- current_value + x[[current_object, "v"]]
      best_combination <- append(best_combination, x[[current_object, "r"]])
    }
    else {
      break
    }
